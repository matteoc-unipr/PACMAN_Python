'''
@author  Matteo Costella / Gianmarco Bigliardi
@This program works only with a modified version of g2d and actor

BEFORE USE, YOU HAVE TO REPLACE THE DIRECTORY OF THE FONT IN g2d_mod.draw_text() WITH THE DIRECTORY OF emulogic.ttf FONT ON YOUR COMPUTER!!!
'''
...
class PacMan(Actor):
    ...
    def control(self, keys):
        ...
        if "ArrowUp" in keys and in_wall(self._x, self._y - self._speed) == False:
            self._dx = 0                 #up
            self._dy = -self._speed
        elif "ArrowDown" in keys and in_wall(self._x, self._y + self._speed) == False:
            self._dx = 0
            self._dy = self._speed       #down
        elif "ArrowLeft" in keys and in_wall(self._x - self._speed, self._y) == False:
            self._dx = -self._speed
            self._dy = 0                 #left  
        elif "ArrowRight" in keys and in_wall(self._x + self._speed, self._y) == False:
            self._dx = self._speed
            self._dy = 0                 #right
        else:
...
class Boardgame():
    ...
    def superpower(self):
        if self._PacMan.sup_count() == True:
            ...
            if self._RedGhost.superpower() == True:
                ...
                self._RedGhost = Ghost(self._arena, (112, 88), 0)  
            if self._BlueGhost.superpower() == True:
                self._BlueGhost.remove_ghost()
                self._BlueGhost = Ghost(self._arena, (112, 88), 2) 
    ...
    def score(self):
        '''
        After a certain time or when pacman lose a live, it lose points
        '''
        if (self._time - self._arena.count() // 30) == 250:
            return self._score - 300
        if (self._time - self._arena.count() // 30) == 150:
            return self._score - 500
        if (self._time - self._arena.count() // 30) == 100:
            return self._score - 500             
...
class PacGui():
    ...
    def tick(self):        
        ...
        scores = "Score: " + str(self._game.score() + self._game.hero().points()) 
        best_score = "Best Score: " + str(self._bestscore)
        self._score = self._game.score() + self._game.hero().points()
        
        g2d.set_color((255, 255, 255))
        g2d.draw_text(name + " ", (5, 10), 8)
        g2d.draw_text(lives + " ", (5, 300), 8)
        g2d.draw_text(toplay + " ", (150, 300), 8)
        g2d.draw_text(scores + " ", (140, 30), 7)
        g2d.draw_text(best_score + " ", (8, 30), 7)
        ...
        if self._game.hero().sup_count():
            g2d.draw_image_clip("PacMan_sprites_mod.png", (208, 0), (16, 16), (180, 8))              #INDICATES WHEN SUPERPOWER ARE ACTIVE ON GUI
        
        if self._game.game_over():
            if self._score > self._bestscore:
                with open("BESTSCORE.txt", "w") as new_bestscore:
                    print(self._score, file=new_bestscore)
                g2d.alert(("Game over!" " " "   New Best Score!!! ", self._score))
            else:
                g2d.alert(("Game over!" " " "   Your score is  ", self._score))
            ...
            if self._score > self._bestscore:
                with open("BESTSCORE.txt", "w") as new_bestscore:
                    print(self._score, file=new_bestscore)
                g2d.alert(("Game won!" " " "   New Best Score", self._score))
            else:
                g2d.alert(("Game won!" " " "   Your score is", self._score))
            g2d.close_canvas()
